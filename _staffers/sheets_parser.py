...
sheet = file.open("Data 100 Summer 2024 Onboarding (Responses)")  # open sheet
...
def attribute_parser(row):
    ...
    attributes["sid"] = row[3] # Update from fa23, row[9] is returner or not
    attributes["name"] = row[7].strip()
    print("Name is", attributes["name"])
    attributes["pronouns"] = row[8]
    attributes["photo_name"] = attributes['name'].replace(' ', '_')
    print("Photo name is", attributes["photo_name"])
    attributes["website"] = row[10]
    attributes["bio"] = row[11].replace('\n', '').replace('’', "'")
    attributes["role"] = assign_role(row[12])
...
def main():
    ...
    for i in range(2, 20): #modify the second number depending on the number of rows in the sheet.
        ...
        if attributes['role'] == "Instructor":
            file.write('---\n'
                ...
                + 'photo: http://ds100.org/su24/resources/assets/staff_pics/' + get_photo_location(photos, attributes) + '\n'
